...
- **Erros explícitos.** Nada de `throw` silencioso: toda execução devolve um [`Result<T, E>`](#resultt-e) que o consumidor é obrigado a tratar.
...
// Pipeline previsão → detecta alerta → push (short-circuit no primeiro erro)
...
Saída esperada (o exemplo imprime em inglês) — cada linha mostra um padrão de resiliência salvando o dia (retry, fallback ao secundário, fallback ao cache):

```
━━━ 1. Resilient forecast per city ━━━
  ✔ Lisbon: 24°C via OpenSky          (retry: OpenSky falhou 2x, acertou na 3ª)
